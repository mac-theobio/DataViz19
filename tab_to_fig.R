...
get_data_from_PDF <- FALSE

## text from table 5.5
dd <- read.table(header=TRUE,check.names=FALSE,text="
...
sim1 0.10 est  0.0578 0.7823 0.2769 0.4558 0.1879 0.5029 0.2325 0.4794
sim1 0.10 sd   0.0116  0.0412 0.0895 0.2147 0.0392 0.109 0.0882 0.139
sim1 0.20 est  0.0674  0.7523 0.2311 0.5615 0.3108 0.2975 0.2963 0.3703
sim1 0.20 sd   0.0335  0.1082 0.0604 0.1052 0.0552 0.1387 0.0541 0.1209
sim1 0.30 est  0.0746  0.7267 0.2369 0.5605 0.4265 0.2461 0.3936 0.2825
sim1 0.30 sd  0.0309 0.099 0.0576 0.075 0.1155 0.1705 0.0824 0.1374
...
sim2 0.2 est 0.1240 0.5722 0.3103 0.4056 0.3153 0.3081 0.3354 0.294
...
sim3 0.10 est 0.0556  0.8316 0.2386 0.5548 0.0878 0.7565 0.2969 0.3988
sim3 0.10 sd  0.1049 0.1979 0.1161 0.2822 0.0367 0.0503 0.1256 0.2891
sim3 0.2 est  0.0744 0.7629 0.246 0.3157 0.3251 0.3157 0.2673 0.4629 
sim3 0.2 sd 0.1010  0.1855 0.0793 0.2448 0.1154 0.2448 0.0673 0.1664
...
sim4 0.10 est 0.3004 0.2011 0.4401 0.1519 0.2518 0.2938 0.4048 0.1842
sim4 0.10 sd 0.1066  0.1593 0.0608 0.075 0.0826 0.1387 0.0689 0.0911
sim4 0.20 est  0.2703  0.2375 0.4359 0.1306 0.4323 0.0829 0.4395 0.0782
sim4 0.20 sd  0.0859  0.1266 0.0743 0.0812 0.0589 0.0857 0.0515 0.0964
...
sim5 0.10 est 0.072  0.7346 0.2545 0.5235 0.1986 0.4692 0.2403 0.4556
sim5 0.10 sd 0.0257  0.0872 0.0879 0.1428 0.0476 0.1087 0.0646 0.0896
sim5 0.20 est 0.0766  0.7239 0.2459 0.5493 0.3454 0.2477 0.2975 0.3767
sim5 0.20 sd 0.0445 0.1317 0.0894 0.1419 0.0808 0.1929 0.065 0.1543
sim5 0.30 est 0.1064  0.6268 0.2395 0.5281 0.3915 0.2961 0.3983 0.2692
sim5 0.30 sd 0.0455   0.1366 0.0739 0.1067 0.0912 0.1328 0.0705 0.1217
...
sim6 0.10 est 0.2573  0.2515 0.4155 0.1921 0.3305 0.1857 0.4068 0.176
sim6 0.10 sd 0.0672 0.1093 0.0693 0.0857 0.058 0.0935 0.1079 0.0958
sim6 0.20 est 0.2501  0.2613 0.3865 0.1818 0.5618 0.0216 0.5625 0.006
sim6 0.20 sd 0.0588 0.1072 0.0655 0.0869 0.0497 0.0377 0.0433 0.0146
sim6 0.30 est 0.2597  0.2442 0.4205 0.1773 0.4992 0.0634 0.4923 0.1094
sim6 0.30 sd 0.0626 0.1064 0.1032 0.0745 0.0903 0.0811 0.0607 0.0676
")

if (get_data_from_PDF) {
    tt4 <- tabulizer::extract_areas("tab.pdf")[[1]]
    m <- matrix(parse_number(c(tt4)), ncol=ncol(tt4))
    nms <- c("dataset","r","type","MGHD.ERR","MGHD.ARI",
             "MST.ERR","MST.ARI","MI/MGHD.ERR","MI/MGHD.ARI",
             "MI/MST.ERR","MI/MST.ARI")
    xcols <- expand.grid(type=c("est","sd"),r=c(0.05,0.1,0.2,0.3),
                         dataset=paste0("sim",1:6))[3:1]
    dd_pdf <- setNames(data.frame(xcols,m),nms)
    ## check equality with hand-coded version ...
    all.equal(dd_pdf,dd)
    dd <- dd_pdf
}
...
dd3 <- (dd2
    %>% full_join(simtab,by="dataset")
    %>% mutate(labs=sprintf("%s/%s",distribution,covstruc),
               labs=forcats::fct_inorder(labs))
)
...
gg1 <- ggplot(dd3,aes(factor(r),est,colour=model)) + 
    ...
    geom_ribbon(aes(ymin=est-sd,ymax=est+sd,group=model,fill=model),
                colour=NA,
                alpha=0.3)+
    facet_grid(stat~labs+separation)+
    scale_y_continuous(limits=c(0,1),expand=c(0,0),oob=scales::squish) +
...
grid::grid.draw(OverlappingStripLabels(gg1))

## TO DO: order models (MGHD,MST,MI/MGHD,MI/MST)
